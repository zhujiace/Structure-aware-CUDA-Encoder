...
class CudaASTGraphBatch:
    ...
    def from_snapshots(
        ...
        *,
        ...
        node_flags = torch.zeros((batch_size, max_nodes, 3), dtype=torch.float32, device=device)
        node_positions = torch.zeros((batch_size, max_nodes, 3), dtype=torch.float32, device=device)
        ...
        for batch_index, snapshot in enumerate(snapshots):
            ...
            for node_index, node in enumerate(nodes):
                ...
                node_flags[batch_index, node_index] = torch.tensor(
                    [float(node.named), float(node.has_error), float(node.is_missing)],
                    dtype=torch.float32,
                    device=device,
                )
                ...
                node_positions[batch_index, node_index] = torch.tensor([start, end, span], dtype=torch.float32, device=device)
